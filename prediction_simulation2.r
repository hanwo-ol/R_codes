...
# You can change these non-zero values if you like.
...
rmse_train_all <- numeric(n_simulations) # Model with all predictors
rmse_eval_all <- numeric(n_simulations)

rmse_train_true <- numeric(n_simulations) # Model with only true predictors
rmse_eval_true <- numeric(n_simulations)

rmse_train_plus1 <- numeric(n_simulations) # Model with true + 1 noise predictor
...
for (i in 1:n_simulations) {
  ...
  pred_train_all <- predict(model_all, newdata = train_df)
  pred_train_true <- predict(model_true, newdata = train_df)
  pred_train_plus1 <- predict(model_plus1, newdata = train_df)
  ...
  rmse_train_all[i] <- sqrt(mean((train_df$Y - pred_train_all)^2))
  rmse_train_true[i] <- sqrt(mean((train_df$Y - pred_train_true)^2))
  rmse_train_plus1[i] <- sqrt(mean((train_df$Y - pred_train_plus1)^2))
  
  # RMSE on evaluation data
...
plot(x = rmse_train_all, y = rmse_eval_all,
     ...
     xlab = "Training RMSE",
     ylab = "Evaluation RMSE",
     pch = 19, col = "dodgerblue",
     xlim = range(c(rmse_train_all, rmse_eval_all)), # Consistent axes range
     ylim = range(c(rmse_train_all, rmse_eval_all)))
...
mtext("Training RMSE vs. Evaluation RMSE (50 Simulations)", outer = TRUE, cex = 1.5)
